    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">{!! trans('modals/modals.resetPwd_title') !!}</h4>
            </div>
            <form id="emailForm" class="x-form"  method="POST" action="{{ route('sendResetLinkEmail') }}">
                    {{ csrf_field() }}
                    <label for="email" class="x-label email-label">{!! trans('modals/modals.resetPwd_label') !!}</label>
