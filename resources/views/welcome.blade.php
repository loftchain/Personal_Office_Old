    ...
    <div class="home-body">
        ...
        <section class="upper-part">
            ...
            <p class="your-entered-as"> @lang('home/home.logged_in_as')
                <span>{{ obfuscate_email('User@gmail.com') }}</span></p>
            <br/>
...
@section('script')
    <script>
      ...
      wid_obj.token_sold.html(data['token_sold'].toLocaleString());
      wid_obj.token_cap.html(data['token_cap'].toLocaleString());
      ...
      if (time < ico_start) { //before pre-ICO
        ...
        current_round = '{{ __('home/widget.round_1') }}';
        diff = ico_end - time;
        wid_obj.currentDay.html(daysInUTC(ico_length) - daysInUTC(diff) +1);
        wid_obj.allDay.html(Math.floor(ico_length / (1000 * 60 * 60 * 24)) +1);
        wid_obj.is_live.html('{{__('home/widget.isLive')}}');
        wid_obj.counter_title.html('{{__('home/widget.the_end')}}' + '{{__('home/widget.round_1a')}}' + '{{__('home/widget.the_end_in')}}');
      ...
      wid_obj.pi_interval.html(daysInUTC(pre_ico_end - pre_ico_start ) + 1 + '{{ __('home/widget.days') }}');
      wid_obj.i_start.html(wid_obj.getSmallDate(ico_start));
      wid_obj.i_end.html(wid_obj.getSmallDate(ico_end));
      wid_obj.i_interval.html(daysInUTC(ico_end - ico_start ) + 1 + '{{ __('home/widget.days') }}');
      ...
      function roundSlider(round) {
        switch (round) {
          case 'before':
            if (daysInUTC(ico_start - time) > 20) {
              wid_obj.i_inner.width('0%');
              wid_obj.round_arrow_before.css({'left': 'calc(0% - 3px)'});
              wid_obj.round_arrow_after.css({'left': 'calc(0% - 3px)'});
            } else if (daysInUTC(ico_start - time) === 0) {
              wid_obj.i_inner.width('10%');
              wid_obj.round_arrow_before.css({'left': 'calc(10% - 3px)'});
              wid_obj.round_arrow_after.css({'left': 'calc(10% - 3px)'});
            } else {
              interval = daysInUTC(ico_start - time);
              percent_val = 10 - interval*5/8;
              wid_obj.i_inner.width(percent_val + '%');
              wid_obj.round_arrow_before.css({'left': 'calc(' + percent_val + '% - 3px)'});
              wid_obj.round_arrow_after.css({'left': 'calc(' + percent_val + '% - 3px)'});
            }
            break;
//			  case 'p-i':
//				  console.log('p-i');
...
//				  break;
          ...
          case 'i':
            var interval = daysInUTC(ico_end - ico_start);
            var now_left = daysInUTC(ico_end - time);
            var percent_val = 90 / interval;
            var between_val = 10 + (interval - now_left) * percent_val;
