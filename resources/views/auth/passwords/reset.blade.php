...
@section('content')
  <div class="reset-body">
    <h3>{!! trans('auth/resetPwd.resetPwd_title') !!}</h3>
    <form id="resetForm"  class="x-form" method="POST" action="{{ route('password.request') }}">
      {{ csrf_field() }}
      <input type="hidden" name="token" value="{{ $token }}">
      <label for="email" class="email-label">{!! trans('auth/resetPwd.resetPwdEmail_label') !!}</label>
      ...
      <label for="password" class="password-label">{!! trans('auth/resetPwd.createNewPwd_label') !!}</label>
      ...
      <label for="password_confirmation" class="password-label">{!! trans('auth/resetPwd.confirmNewPassword_label') !!}</label>
      ...
      <button type="submit" class="login-btn modal-btn reusable-btn"> <i class="small-spinner fa fa-circle-o-notch fa-spin fa-lg fa-fw"></i>
        {!! trans('auth/resetPwd.confirmEnter_btn') !!}
      </button>
    </form>
  </div>
@endsection