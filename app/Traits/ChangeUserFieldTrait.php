...
trait ChangeUserFieldTrait
{
    ...
    public function sendNewEmail($data, $timeout, $key, $data_old = null)
    {
        ...
        for ($i = 0; $i < 3; $i++) {
            ...
            $email_from_name = env('UNISENDER_FROM_NAME');
            $email_from_email = env('UNISENDER_FROM_EMAIL');
            ...
            $email_subject = ($key == 'email') ? 'New email at '.env('APP_NAME') : (($key == 'password') ? 'New password at '.env('APP_NAME') : 'Reset password at '.env('APP_NAME'));
            $list_id = env('UNISENDER_ID');;
            $request = [
                'api_key' => $api_key,
